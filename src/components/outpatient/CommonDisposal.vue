...
<script>
  import {
    listCommonDisposal,
    deleteCommonDisposal,
    deleteCommonDisposalsList,
    updateCommonDisposal,
    addCommonDisposal,
  ...
  export default {
    ...
    methods: {
      ...
      //删除对象
      handleDelete(val) {
        let id = {'commonDisposalID': val};
        this.$confirm('确认删除？')
          .then(_ => {
            deleteCommonDisposal(id).then((res) => {
                if (res.status === 200) {
                  let data = res.data;
                  if (data.status === 'OK') {
                    this.freshInfo();
      ...
      deleteByChoose() {
        this.$confirm('确认批量删除？')
          .then(_ => {
            ...
            deleteCommonDiposalsList(params).then((res) => {
                if (res.status === 200) {
                  let data = res.data;
                  if (data.status === 'OK') {
                    this.freshInfo();
          ...
          if (valid) {
            if (formName === 'editForm')
              this.$confirm('确认修改常用检查检验项目信息？')
                .then(_ => {
                  updateCommonDisposal(this.editForm).then((res) => {
                      if (res.status === 200) {
                        let data = res.data;
                        if (data.status === 'OK') {
                          this.$message({
              ...
              this.$confirm('确认添加常用检查检验项目信息？')
                .then(_ => {
                  let params={disposalID:this.addForm.fMedItemID};
                  addCommonDisposal(params).then((res) => {
                      if (res.status === 200) {
                        let data = res.data;
                        if (data.status === 'OK') {
                          this.freshInfo();
